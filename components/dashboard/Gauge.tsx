...
export default function Gauge({
  ...
  const ticks = [0, 25, 50, 75, 100].map((t) => {
    ...
    return {
      ...
      x2: 100 + 83 * Math.cos(a), y2: 100 - 83 * Math.sin(a),
    };
  });

  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%", maxWidth: 300 }}>
      <div style={{ fontSize: ".68rem", textTransform: "uppercase", letterSpacing: ".07em", color: "var(--muted)", fontWeight: 800, marginBottom: 4, textAlign: "center" }}>
        {caption}
      </div>
      <svg viewBox="0 0 200 116" style={{ width: "100%" }}>
        <defs>
        ...
        <text x="12" y="113" textAnchor="start" fontSize="9" fill="var(--muted)">{leftLabel}</text>
        <text x="188" y="113" textAnchor="end" fontSize="9" fill="var(--muted)">{rightLabel}</text>
      </svg>
      <div style={{ fontSize: "1.9rem", fontWeight: 800, color: zone, marginTop: -6, lineHeight: 1 }}>{centerText}</div>
      {footer && <div style={{ fontSize: ".76rem", color: "var(--muted)", marginTop: 6, textAlign: "center" }}>{footer}</div>}
    </div>
  );
}
